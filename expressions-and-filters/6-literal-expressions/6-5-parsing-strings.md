...
那我们到底要在 `while` 循环语句中干什么呢？有两件事：如果当前字符不是引号，我们应该把它拼接到结果字符串中去。如果是引号，说明该字符串已经结束了，我们要把当前拼接好的字符串放到 token 数组中去，然后结束循环。循环语句执行完之后，如果程序还在执行 `readString` 函数，我们要抛出一个异常，因为这意味着表达式都读取完了但字符串没有被终止：
